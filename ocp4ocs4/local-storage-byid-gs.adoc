...
[IMPORTANT]
====
Prior to OCS 4.3 GA, *skip* the steps above to create openshift-storage namespace. Instead use https://raw.githubusercontent.com/red-hat-storage/ocs-training/ocs-goldman/ocp4ocs4/deploy-with-olm.yaml[deploy-with-olm.yaml] and execute `oc create -f deploy-with-olm.yaml`.
...
[IMPORTANT]
====
Prior to OCS 4.3 GA, the lib-bucket-provisioner `csv` and `deployment` will need to be edited and the image *kubernetes/pause* will need to be changed to *quay.io/crio/pause@sha256:21426d7935da8cda014d98236842a0c2558cce328e69187d3a6b8d08e30b520d*. Edit the `csv` first and then the lib-bucket-provisioner `deployment` second to replace the image:sha name.
====
